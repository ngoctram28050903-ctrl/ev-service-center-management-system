...
export const createWorkOrder = async (req, res) => {
  ...
  try {
    ...
    if (existingWorkOrder) {
      return res
        .status(400)
        .json({ message: "Work order already exists for this appointment" });
    }
    ...
    res
      .status(201)
      .json({ data: newWorkOrder, message: "Work order created successfully" });
  } catch (err) {
...
export const updateWorkOrder = async (req, res) => {
  try {
    ...
    await publishToExchange("workorder_events", {
      ...
      payload: order, // Gửi toàn bộ đối tượng đã cập nhật
    });
...
export const getWorkOrderByAppointmentId = async (req, res) => {
  try {
    ...
    if (!workOrder)
      return res
        .status(404)
        .json({ message: "Work order not found for this appointment" });
    res.json(workOrder);
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
      ...
      rows.map(async (item) => {
        ...
        try {
          if (itemData.workOrder?.appointmentId) {
            ...
            const appointment = await bookingClient.getAppointmentById(
              itemData.workOrder.appointmentId
            );

            if (appointment?.vehicleId) {
              // Get vehicle details
              const vehicle = await vehicleClient.getVehicleById(
          ...
          if (itemData.assignedToUserId) {
            try {
              const assignedUser = await userClient.getUserById(
                itemData.assignedToUserId
              );
              itemData.assignedUser = assignedUser;
            } catch (error) {
              console.error(
                "Error fetching assigned user details:",
                error.message
              );
          ...
          console.error(
            "Error fetching appointment/vehicle details:",
            error.message
          );
...
export const getTaskStats = async (req, res) => {
  try {
    ...
    const totalStats = await ChecklistItem.findAll({
      attributes: [[sequelize.fn("COUNT", sequelize.col("id")), "totalTasks"]],
      raw: true,
    });
